...
import REGNetv2.REGNetv2.utils_add as utils
import REGNetv2.REGNetv2.contrast.model_utils as contrast_utils
from REGNetv2.REGNetv2.contrast.tdgpd.models.build_model import build_model 
from REGNetv2.REGNetv2.contrast.tdgpd.utils.file_logger import file_logger_noselect as file_logger_cls
from REGNetv2.REGNetv2.contrast.tdgpd.yacs_config import load_cfg_from_file
...
class GraspDetector:
    def __init__(self, config):
        config.cuda = config.cuda if torch.cuda.is_available else False
        self.config = config

        self.gpu_num = config.gpu
        torch.cuda.set_device(self.gpu_num)
        self.method = config.method
        self.all_points_num = config.all_points_num
    ...
    def _init_model(self):
        ...
        self.width, self.height, self.depth = self.config.eval_width, 0.010, 0.06
        grasp_channel = 9
        radius = 0.06
        max_radius = 0.10
        ...
        if 'multigrasp' in self.method or 'regnet' == self.method:
            ...
            model_machine = utils.ModelInit('test', self.config.load_model, 'class_01', model_params, \
            ...
            model_machine = contrast_utils.ModelInit('test', self.config.load_model, model_params, grasp_points_num, point_channel, class_num)
            self.model, _, _, _ = model_machine.construct_model()
            ...
            model_machine = contrast_utils.GPDModelInit('test', self.config.load_model, model_params, grasp_points_num, project_chann)
            self.model, _, _, _ = model_machine.construct_model()
            ...
            checkpoint = torch.load(self.config.load_model, map_location='cuda:{}'.format(self.gpu_num))
            new_model_dict = {}
    ...
    def get_view_init(self):
        euler_deg = self.config.camera_transform["euler_deg"]  # XYZ 歐拉角 (deg)
    ...
    def to_torch(self, pc):
        ...
        data_width = torch.tensor([self.config.eval_width])
    ...
    def method_multigrasp(self, pc):
        ...
        if len(pre2_grasp) >= 1:
            ...
            if len(keep_grasp) > 0:
                ...
                if len(grasp_mat) > 0:
                    ...
                    grasp_num = min(len(grasp_mat), self.config.vis_grasp_nums)
                    ...
                    # grasp_save_path = os.path.abspath( self.base_path + f"/{self.config.exp_name}/grasp_info.p")
                    # if not os.path.exists(self.base_path + f"/{self.config.exp_name}"):
                    #     os.makedirs(self.base_path + f"/{self.config.exp_name}")
        ...
        return final_grasp, final_grasp_20, data
    ...
    def generate_grasp_from_file(self, pc, pc_path):
        ...
        self.base_path  = self.config.vis_output
        ...
        # with open(grasp_save_path, 'wb') as file:
        #     pickle.dump(data, file)
        vis_save_path = self.config.vis_output + self.config.exp_name + '/'
        if self.config.vis_lot_grasp:
    ...
    def generate_grasp(self, pc):
        ...
        if 'multigrasp' in self.method or 'regnet' == self.method:
            grasp, grasp5, data = self.method_multigrasp(pc)
        elif 'gpd' in self.method or self.method=='random':
            grasp = self.method_gpd(pc)
        elif self.method == 's4g':
            grasp = self.method_s4g(pc)
        
        #TODO: 加個篩選機制來篩掉一些會collision的姿勢?
    ...
    def _local_to_global_transformation_mat(self):
        ...
        if self.config.camera == 'kinect2':
            euler_deg = self.config.camera_transform['euler_deg']
            translation = self.config.camera_transform['translation']
        ...
        elif self.config.camera == 'kinectdk_right':
        ...
        elif self.config.camera == 'kinectdk_left':
    ...
    def _get_show_pc(self, pc, real_data):
        ...
        if real_data:
            ...
            if 'kinectdk' in self.config.camera:
...
def main(config):
    import glob
    pc_paths = glob.glob(config.pc_paths)
    destination_folder = config.vis_output + config.exp_name + '/'
    os.makedirs(destination_folder, exist_ok=True)
    
    GDetector = GraspDetector(config)
