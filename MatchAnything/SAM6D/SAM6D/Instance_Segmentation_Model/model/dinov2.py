...
import logging
import numpy as np
from MatchAnything.SAM6D.SAM6D.Instance_Segmentation_Model.utils.bbox_utils import CropResizePad, CustomResizeLongestSide
from torchvision.utils import make_grid, save_image
from MatchAnything.SAM6D.SAM6D.Instance_Segmentation_Model.model.utils import BatchedData
...
class CustomDINOv2(pl.LightningModule):
    ...
    @torch.no_grad()
    def forward_by_chunk_v2(self, processed_rgbs, masks=None):
        batch_rgbs = BatchedData(batch_size=self.chunk_size, data=processed_rgbs)
        ...
        features = BatchedData(batch_size=self.chunk_size)
        for idx_batch in range(len(batch_rgbs)):
            batch_rgb = batch_rgbs[idx_batch]
            if batch_masks is not None:
                batch_mask = batch_masks[idx_batch]
                feats = self.compute_masked_patch_feature(batch_rgb, batch_mask)
            else:
                feats = self.compute_patch_feature(batch_rgb)
            features.cat(feats)
        return features.data
    ...
    @torch.no_grad()
    def compute_patch_feature(self, images, masks=None):
        # without preprocess
        if images.shape[0] > self.chunk_size:
            features = self.forward_by_chunk_v2(images, masks)
        else:
            features = self.model(images, is_training=True)["x_norm_patchtokens"]
            features = F.normalize(features, dim=-1)  # L2 normalize 所有 patch token
        return features
